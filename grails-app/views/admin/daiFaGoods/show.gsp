...
<html>
  ...
  <body>
    <div class="container body">
      ...
      <div id="admin_shuomingModal2" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
          <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
          <h3 id="myModalLabel">填写缺货说明</h3>
      </div>
      <div class="modal-body">     
        <g:form class="admin_shuomingForm2 form-horizontal" data-validate="parsley"  action="noQuehuo" target="innerFrame">
         <input type='hidden' name='id' id='id' value="${daiFaGoods.id}">
          <div class="control-group">
            <label class="control-label">缺货说明</label>
            <div class="controls">
          ...
          <div class="control-group">
            <div class="controls">
              <button type="button" class="btn btn-primary btn-large" onclick="toActionFormCom('admin_shuomingForm2')" >确定</button>
            </div>
          </div>
          </g:form>
      </div>
  ...
  <h4>操作记录</h4>
   <div class="bs-docs-example">
    ...
    <g:if test="${daiFaGoods.status == '3' &&  session.loginPOJO.user.role == 'admin'}">
        <a href="javascript:void(0)" onclick="toActionCom({goods_id:'${daiFaGoods.id}'},'<%=request.getContextPath()%>/adminDaiFaGoods/bukuan','确认补款吗，此商品单件补款价格是${daiFaGoods.actual_price - daiFaGoods.price}元')"  class="btn btn-large btn-primary">确认需要补款</a>（经确认,该商品真实需要补款才能拿货,请在此处操作确认补款,并输入金额）
        <br/>
        <br/>
    ...
    <g:if test="${daiFaGoods.status == '8' &&  session.loginPOJO.user.role == 'admin'}">
        ...
        <a href="#admin_shuomingModal2" data-toggle="modal"   class="btn btn-large btn-danger">取消缺货</a>
    </g:if>
    <g:if test="${daiFaGoods.status == '2' &&  session.loginPOJO.user.role == 'admin'}">
        ...
        <a class="btn btn-large btn-danger" href="javascript:void(0)" onclick="toActionCom({goods_id:'${daiFaGoods.id}'},'<%=request.getContextPath()%>/adminDaiFaGoods/buhege','确认提交此商品不合格吗？')">不合格</a>

    </g:if>
    <g:link controller="adminDaiFaGoods" action="list"  params="${params}" class=" btn btn-large">返回</g:link>
  </fieldset>
...
<iframe id="innerFrame" name="innerFrame" height="0" frameborder="0"></iframe>
         <g:form  action="toOrderGoods" params="${params}"  method="post" class="commonListForm">
        </g:form >

...
def jcaptchaService
    def index() {
        ...
        def stalls = DaiFaGoods.executeQuery("select sum(num) as tot ,market,floor,stalls from DaiFaGoods  group by market,floor, stalls order by tot desc  limit 0,10 ");


        def daifaGoods =  DaiFaGoods.findAllByDateCreatedBetweenAndStatusInList(new Date()-10,new Date(),["1","2","7","9"],[max: 100, sort: "dateCreated", order: "desc", offset: 0]);
        def map = [daifaGoods:daifaGoods,stalls:stalls.getAt(0..15) ]
